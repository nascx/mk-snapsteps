    ...
    existsThisIT = exports.existsThisIT = (path) => {
      try {
        return new Promise(async (resolve, reject) => {
          ...
          await _chunkTUARQIGFjs.db.query(q, [path], (err, data) => {
            if (err) {
              reject(err);
            }
    ...
    insertIT = exports.insertIT = (path, name) => {
      try {
        return new Promise(async (resolve, reject) => {
          ...
          await _chunkTUARQIGFjs.db.query(q, [path, name], (err, data) => {
            if (err) {
              reject(err);
            }
    ...
    existsThisQAFile = exports.existsThisQAFile = (code) => {
      try {
        return new Promise(async (resolve, reject) => {
          ...
          await _chunkTUARQIGFjs.db.query(q, [code], (err, data) => {
            if (err) {
              reject(err);
            }
            if (data && data.length > 0) {
    ...
    insertQAFile = exports.insertQAFile = (code, title, path) => {
      try {
        return new Promise(async (resolve, reject) => {
          ...
          await _chunkTUARQIGFjs.db.query(q, [code, title, path], (err, data) => {
            if (err) {
              console.log(err);
              reject(err);
            }
...
//# sourceMappingURL=chunk-C5GCJCLC.js.map