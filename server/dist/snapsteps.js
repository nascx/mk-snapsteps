...
var createList = async (req, res) => {
  try {
    console.log("Fun\xE7\xE3o chamada!");
    const filePath = import_node_path.default.resolve(__dirname, `../../00_engineering_lists/${req.file?.originalname}`);
...
var uploadProductionLists = async (req, res) => {
  try {
    const filePath = import_node_path2.default.resolve(__dirname, `../../00_production_lists/${req.file?.originalname}`);
    const jsonData = convertExcelToJsonWithoutAlterLine(filePath);
